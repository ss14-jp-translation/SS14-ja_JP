...
generic-misconfigured = サーバーの設定が間違っており、プレイヤーを受け入れていません。サーバー所有者に連絡し、後で再試行してください。

ipintel-server-ratelimited = このサーバーは外部認証のセキュリティシステムを使用しており、認証の上限に達しています。サーバーの管理チームに連絡し、後で再試行してください。
ipintel-unknown = このサーバーは外部検証のセキュリティシステムを使用していますが、エラーが発生しました。サーバーの管理チームに連絡してサポートを受けてください。
ipintel-suspicious = データセンターまたはVPN経由で接続しているようです。管理上の理由により、VPN接続でのプレーは許可しておりません。このような場合、サーバーの管理チームまでお問い合わせください。

hwid-required = クライアントがハードウェアIDの送信を拒否しました。管理チームまでご連絡ください。
